  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-slate-900">Job Recommendations</h1>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
          ...
          <div className="relative">
            ...
            <select
              ...
              className="w-full pl-9 pr-3 py-2 text-xs rounded-xl border border-slate-200 bg-slate-50 text-slate-700 focus:outline-none focus:ring-2 focus:ring-brand-500"
            >
      ...
      {isLoading ? (
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-12 text-center">
          ...
          <h3 className="text-sm font-bold text-slate-800">No jobs match your current filters</h3>
          ...
          <button
            ...
            className="mt-4 px-4 py-2 text-xs font-semibold text-brand-600 bg-brand-50 rounded-lg hover:bg-brand-100"
          >
            Reset Filters
          </button>
        </div>
          ...
          <div className="space-y-4 text-xs">
            <div className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-100 pb-3">
              <div>
                ...
                <p className="text-slate-500">{selectedJob.location} • {selectedJob.job_type} • {selectedJob.experience_level}</p>
              </div>
              <div className="text-right">
                <span className="text-xs font-bold text-slate-800">{selectedJob.salary_range}</span>
                <p className="text-[11px] font-semibold text-emerald-600">{selectedJob.match_percentage}% Profile Fit</p>
              </div>
            </div>
            ...
            {selectedJob.why_matched && (
              <div className="rounded-xl bg-blue-50 border border-blue-200 p-3 text-blue-950">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2">
              <div className="rounded-xl bg-emerald-50/70 border border-emerald-200 p-3">
                <p className="font-bold text-emerald-800 mb-1.5">Matching Competencies ({selectedJob.matching_skills?.length || 0})</p>
                <div className="flex flex-wrap gap-1">
                  {selectedJob.matching_skills?.map((s) => (
                    <span key={s} className="bg-white text-emerald-700 border border-emerald-200 px-2 py-0.5 rounded font-medium text-[11px]">
                      ✓ {s}
                    </span>
                  ))}
                </div>
              ...
              <div className="rounded-xl bg-rose-50/70 border border-rose-200 p-3">
                <p className="font-bold text-rose-800 mb-1.5">Missing Skills ({selectedJob.missing_skills?.length || 0})</p>
                <div className="flex flex-wrap gap-1">
                  {selectedJob.missing_skills?.map((s) => (
                    <span key={s} className="bg-white text-rose-700 border border-rose-200 px-2 py-0.5 rounded font-medium text-[11px]">
                      + {s}
                    </span>
                  ))}
                </div>
